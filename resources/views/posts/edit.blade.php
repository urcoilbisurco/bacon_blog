...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-edit"></i> Posts / Edit #{{$post->id}}</h1>
    </div>
@endsection
            ...
            <form action="{{ route('posts.update', $post->id) }}" method="POST">
                ...
                <div class="form-group @if($errors->has('title')) has-error @endif">
                    ...
                    <div class="form-group @if($errors->has('image_path')) has-error @endif">
                       <label for="image_path-field">Image_path</label>
                    <input type="text" id="image_path-field" name="image_path" class="form-control" value="{{ is_null(old("image_path")) ? $post->image_path : old("image_path") }}"/>
